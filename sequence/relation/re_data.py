...
class REDataset(Dataset):
    ...
    def _get_list(self, jsonstr):
        ...
        for index, spo_list in enumerate(spo_lists):
            ...
            for subject_start in subject_start_poses:
                subject_end = subject_start + len(subject) - 1
                if text[subject_start:subject_start + 1] == object:
                    subject_start_pos = subject_start
                    subject_end_pos = subject_end
                    break
            tag_list[subject_start_pos] = 'B_' + subject_type
            for i in range(subject_start_pos + 1, subject_end_pos + 1):
                tag_list[i] = 'I_' + subject_type
            object_start_poses = self._find_all_index(text, object)
            for object_start in object_start_poses:
                object_end = object_start + len(object) - 1
                if text[object_start:object_end + 1] == object:
                    object_start_pos = object_start
                    object_end_pos = object_end
                    break
            tag_list[object_start_pos] = 'B_' + predicate
            for i in range(object_start_pos + 1, object_end_pos+1):
                tag_list[i] = 'I_' + predicate
        text_list = list(text)
